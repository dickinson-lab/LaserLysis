 ...
 */
   ...
   int pulsedLaserPin = 9;
   // pin connected to the Teem laser

   unsigned long previousMillis = 0; // Stores the last time the laser was fired
   int blinkInterval = 500;
   bool blinkPulsedLaser = false; 
   // when this is true, the pulsed laser will fire shots at an interval specified by "blinkInterval"
 ...
 void loop() {
   if (Serial.available() > 0) {
     ...
     switch (inByte) {
       ...
       // Set digital output - this was modified to allow turning on a pulsed laser.
       case 1 :
          if (waitForSerial(timeOut_)) {
            ...
            // Look in pattern for bit 1, which is a dummy laser to specify a single laser shot.
            if (currentPattern_ & B00000001) {
              //Send a single pulse
              digitalWrite(pulsedLaserPin, HIGH);
              delayMicroseconds(200);
              digitalWrite(pulsedLaserPin, LOW);
              Serial.write( byte(1));

            // Look in pattern for bit 2, which is a dummy laser to fire a shot every 500 ms.
            } else if (currentPattern_ & B00000010) {
              //Start pulsing at regular intervals
              blinkPulsedLaser = true;
              Serial.write( byte(1));
            
            // Look in pattern for bit 4, which is a dummy laser to specify continuous pulsing.
            } else if (currentPattern_ & B00001000) { 
              analogWrite(pulsedLaserPin, 50); // Start pulsing
              Serial.write( byte(1));
            
            //Default is the original code to turn on other lasers.
            } else {
              analogWrite(pulsedLaserPin, 0); // Stop pulsing
              blinkPulsedLaser = false; // Stop pulsing
              if (!blanking_) {
                PORTB = currentPattern_; // No need to worry about accidentally setting pins 8 or 11 high since we'll only get here if bits 1 and 4 are 0
              }
              Serial.write( byte(1));
            }
          }
    ...
    // When blinking mode is on, fire shots at specified intervals
    if (blinkPulsedLaser) {
